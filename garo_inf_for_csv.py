"""
Garofalakis-Kumar L-infinity (Max Error) Wavelet Synopsis Algorithm
...
while minimizing the maximum absolute error (L-infinity norm).
...
2. L-INFINITY ERROR: Minimizes the MAXIMUM error across all points
   - L-inf norm = max(|reconstructed - original|)
   - Unlike L2 (RMS), L-inf focuses on worst-case error
   - Critical for applications where peak errors matter
...
os.makedirs("results_method1_linf", exist_ok=True)
...
@njit(cache=True)
def compute_leaf_errors_linf(incoming: np.ndarray, detail: float, 
                              left_avg: float, right_avg: float,
                              paths: int) -> Tuple[np.ndarray, np.ndarray]:
    """Compute leaf-level L-infinity errors for keep/don't-keep decisions."""
    kerror_all = np.zeros(paths, dtype=np.float64)
    derror_all = np.zeros(paths, dtype=np.float64)
    
    for j in range(paths):
        # KEEP errors - L-infinity: max of absolute errors
        klerror = np.abs(incoming[j] + detail - left_avg)
        krerror = np.abs(incoming[j] - detail - right_avg)
        kerror_all[j] = max(klerror, krerror)
        
        # DON'T KEEP errors - L-infinity: max of absolute errors
        dlerror = np.abs(incoming[j] - left_avg)
        drerror = np.abs(incoming[j] - right_avg)
        derror_all[j] = max(dlerror, drerror)
    
    return kerror_all, derror_all


@njit(cache=True)
def dp_internal_loop_linf(localB: int, paths: int, fullchildB: int,
                          L_error: np.ndarray, R_error: np.ndarray,
                          lenL: int, lenR: int, L_paths: int, R_paths: int):
    """
    Numba-optimized DP loop for internal nodes (L-infinity version).
        ...
        for j in range(paths):
            ...
            if klocalchildB >= 0 and kchildpath < L_paths and kchildpath < R_paths:
                ...
                for leftspace in range(start_left, end_left):
                    ...
                    if 0 <= rightspace < lenR:
                        # L-infinity: take MAX of children's errors
                        error = max(L_error[leftspace, kchildpath], R_error[rightspace, kchildpath])
                        if error < kbesterror:
                            kbesterror = error
            ...
            if dchildpath < L_paths and dchildpath < R_paths:
                ...
                for leftspace in range(start_left, end_left):
                    ...
                    if 0 <= rightspace < lenR:
                        # L-infinity: take MAX of children's errors
                        error = max(L_error[leftspace, dchildpath], R_error[rightspace, dchildpath])
                        if error < dbesterror:
                            dbesterror = error
...
# =============================================================================
# OPTIMIZED GARO CLASS (L-INFINITY VERSION)
# =============================================================================

class GaroOptimizedLinf:
    """Fully optimized Garofalakis-Kumar L-infinity Wavelet Synopsis Algorithm."""
    
    def __init__(self, B: int, csv_file: str = "data/test_data_v2.csv",
        ...
        if self.verbose:
            print(f"\n\t**Garofalakis L_inf Method (OPTIMIZED) ***\n")
    ...
    def _recurse_root(self, write_files: bool = True):
        ...
        self.source = source.copy()  # sparse Haar coefficients (B non-zero entries)
    ...
    def _recurse(self, index: int) -> Tuple[np.ndarray, np.ndarray, np.ndarray, Dict]:
        """Optimized DP recursion using Numba for inner loops (L-infinity version)."""
        depth = self._get_depth(index)
        paths = 1 << (depth + 1)
        ...
        if index < self.N // 2:  # Internal nodes
            ...
            # Use Numba-optimized DP loop (L-infinity version)
            dp_error, dp_keep, dp_leftspace = dp_internal_loop_linf(
            ...
            # Use Numba-optimized error computation (L-infinity version)
            kerror_all, derror_all = compute_leaf_errors_linf(
                incoming, detail, left_avg, right_avg, paths
            )
    ...
    def _write_output_files(self, recon_scaled: np.ndarray):
        ...
        print(f"  Max error: {self.df_output['error'].max():.6f} kWh")
...
def main():
    """L-infinity Wavelet Compression with Simple Denoising - OPTIMIZED."""
    print("="*60)
    print("ADAPTIVE L-INFINITY WAVELET COMPRESSION (OPTIMIZED + NUMBA JIT)")
    print("="*60)
    
    # Configuration
    df = pd.read_csv("data/test_data_v2.csv")
    ...
    # ------------------------------------------------------------------
    # 4. L-infinity target: 15% of maximum point value
    # ------------------------------------------------------------------
    clean_for_target = clean_signal[clean_mask]
    data_max = np.max(np.abs(clean_for_target))
    target_error_pct = 15.0  # 15% of maximum point
    target_error = (target_error_pct / 100.0) * data_max
     
    coeff_pct_start = 0.078
    ...
    print(f"   Target L-inf error: {target_error:.4f} ({target_error_pct}% of max={data_max:.2f})")
    ...
    # ------------------------------------------------------------------
    linf_error_clean = None
    best_final_reconstructed_with_outliers = None
    
    print("🚀 Initializing optimized Garo L-inf (with Numba JIT compilation)...")
    print("   (First run includes JIT compilation overhead)\n")
    
    # Create Garo ONCE - transform happens here
    gk = GaroOptimizedLinf(B=max(1, int(coeff_pct_start * N)), scale_factor=1.0, 
    ...
    while coeff_pct <= coeff_pct_max and iteration <= 100:
        ...
        # Stopping error on CLEAN - L-infinity (max absolute error)
        err_clean = clean_signal - clean_reconstructed
        err_clean_non_outliers = err_clean[clean_mask]
        linf_error_clean = np.max(np.abs(err_clean_non_outliers))
        error_pct_clean = (linf_error_clean / (data_max + 1e-10)) * 100.0
        ...
        linf_error_original = np.max(np.abs(errors))
        error_pct_original = (linf_error_original / (np.max(np.abs(original)) + 1e-10)) * 100.0
        ...
        results.append({
            ...
            "linf_error_clean": linf_error_clean,
            "linf_error": linf_error_original,
            "mean_abs_error": mae,
            "rmse": rmse,
            "nrmse": nrmse,
        ...
        print(f"   L-inf (clean): {linf_error_clean:.4f} ({error_pct_clean:.2f}%)")
        print(f"   L-inf (original): {linf_error_original:.4f} ({error_pct_original:.2f}%)")
        print(f"   RMSE: {rmse:.4f} kWh")
        print(f"   Target: {target_error:.4f} ({target_error_pct}%)")
        print(f"   Status: {'✅ TARGET MET' if linf_error_clean <= target_error else '❌ TARGET NOT MET'}")
        
        if linf_error_clean <= target_error:
            print(f"\n{'='*60}")
    ...
    if (coeff_pct > coeff_pct_max or iteration > 100) and (linf_error_clean is not None) and (linf_error_clean > target_error):
        print(f"\n{'='*60}")
        print(f"⚠️  Reached coefficient limit.")
        print(f"    Final L-inf error: {linf_error_clean:.4f} (target: {target_error:.4f})")
        print(f"{'='*60}")
    ...
    results_df.to_csv("results_method1_linf/garo_fullsignal_metrics.csv", index=False)
    ...
    if best_final_reconstructed_with_outliers is not None:
        ...
        output_dir = "results_method1_linf"
    ...
    print(f"\n{'='*60}")
    print(f"💾 Results saved to results_method1_linf/")
    print(f"{'='*60}")
...
if __name__ == "__main__":
    ...
    output_dir = Path("results_method1_linf")
